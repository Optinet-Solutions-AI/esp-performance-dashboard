...
function ChartsView() {
  ...
  const brandColor = ESP_COLORS[selectedEsp] || '#7c5cfc'
  ...
  const volumeData = {
    ...
    datasets: [
      ...
      },
      {
        ...
        borderColor: brandColor, backgroundColor: brandColor + '12',
        fill: true, tension: 0.35, pointRadius: 4, pointHoverRadius: 7, borderWidth: 2,
      },
    ],
  }
  ...
  const deliveryRateData = {
    ...
    datasets: [{
      ...
      borderColor: '#00e5c3', backgroundColor: 'rgba(0,229,195,0.08)',
      fill: true, tension: 0.35, pointRadius: 4, pointHoverRadius: 7, borderWidth: 2,
    }],
  }
  ...
  const bounceRateData = {
    ...
    datasets: [{
      ...
      borderColor: '#ff4757', backgroundColor: 'rgba(255,71,87,0.06)',
      fill: true, tension: 0.35, pointRadius: 4, pointHoverRadius: 7, borderWidth: 2,
    }],
  }
  ...
  const charts = [
    ...
    { label: 'Delivery Rate Trend', sublabel: 'Delivered ÷ Sent × 100', data: deliveryRateData, opts: rateOptions, accent: '#00e5c3' },
    { label: 'Open Rate Trend', sublabel: 'Opens ÷ Delivered × 100', data: openRateData, opts: rateOptions, accent: brandColor },
    { label: 'Bounce Rate Trend', sublabel: 'Bounced ÷ Sent × 100', data: bounceRateData, opts: rateOptions, accent: '#ff4757' },
  ]
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className={`text-sm mt-1 ${isLight ? 'text-gray-500' : 'text-[#a8b0be]'}`}>
            KPI trend charts for the selected ESP and date range
          </p>
        </div>
      ...
      {!hasData ? (
        <div className={`${cardClass} p-12 text-center`}>
          ...
          <div className={`text-sm ${isLight ? 'text-gray-500' : 'text-[#a8b0be]'}`}>
            Upload a file via Upload Report to see charts here.
          </div>
        </div>
      ) : (
        <>
          {aggOverall && (
            <div className="grid grid-cols-5 gap-3 mb-5">
              {[
                ...
                <div key={k.label} className={`${cardClass} px-4 py-3`} style={{ borderLeft: `3px solid ${k.accent}` }}>
