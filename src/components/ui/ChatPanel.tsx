      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {SUGGESTED_QUESTIONS.map(q => (
              <button
                ...
                style={{
                  ...
                  color: textColor, cursor: 'pointer', transition: 'background 0.15s',
                }}
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            {msg.role === 'assistant' && !!msg.followups?.length && i === messages.length - 1 && !isLoading && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginLeft: 36 }}>
                {msg.followups.map(q => (
                  <button
                    key={q}
                    onClick={() => onSend(q)}
                    style={{
                      textAlign: 'left', padding: '6px 10px', borderRadius: '999px', fontSize: '12.5px',
                      background: 'transparent', border: `1px solid rgba(0,229,195,0.4)`,
                      color: textColor, cursor: 'pointer', transition: 'background 0.15s',
                    }}
                    onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(0,229,195,0.08)' }}
