...
export default function OgChartsView() {
  ...
  const data: MmData = store.ogData
  const fromIdx = store.ogFromIdx
  const toIdx = store.ogToIdx
  const setRange = store.setOgRange
      ...
      {!hasData ? (
        <div className={`${cardClass} p-12 text-center`}>
          ...
          <div className={`text-sm ${isLight ? 'text-gray-500' : 'text-[#a8b0be]'}`}>
            Upload an Ongage file via Upload Report to see charts here.
          </div>
        </div>
      ) : (
        <>
            ...
            {charts.map(chart => (
              <div key={chart.label} className={`${cardClass} p-4`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="text-sm font-semibold" style={{ color: chart.accent }}>
                      {chart.label}
                    </div>
